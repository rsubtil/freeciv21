...
static bool manual_command(struct tag_types *tag_info)
{
  ...
  for (int imanuals = 0; imanuals < MANUAL_COUNT; imanuals++) {
    ...
    fprintf(doc, "<!-- Generated by lunar_gambit-manual version %s -->\n\n",
            freeciv_datafile_version());

    switch (manuals) {
    case MANUAL_SETTINGS:
      // TRANS: markup ... Freeciv21 version ... ruleset name ... markup
      fprintf(doc, _("%Lunar Gambit %s server options (%s)%s\n\n"),
    ...
    case MANUAL_COMMANDS:
      ...
      fprintf(doc, _("%sLunar Gambit %s server commands%s\n\n"),
    ...
    case MANUAL_TERRAIN:
      // TRANS: markup ... Freeciv21 version ... ruleset name ... markup
      fprintf(doc, _("%sLunar Gambit %s terrain help (%s)%s\n\n"),
              tag_info->title_begin, freeciv21_version(), game.control.name,
              tag_info->title_end);
    ...
    case MANUAL_WONDERS:
      if (manuals == MANUAL_BUILDINGS) {
        // TRANS: markup ... Freeciv21 version ... ruleset name ... markup
        fprintf(doc, _("%sLunar Gambit %s buildings help (%s)%s\n\n"),
                tag_info->title_begin, freeciv21_version(),
                game.control.name, tag_info->title_end);
      } else {
        // TRANS: markup ... Freeciv21 version ... ruleset name ... markup
        fprintf(doc, _("%sLunar Gambit %s wonders help (%s)%s\n\n"),
                tag_info->title_begin, freeciv21_version(),
                game.control.name, tag_info->title_end);
      }
    ...
    case MANUAL_GOVS:
      ...
      // TRANS: markup ... Freeciv21 version ... ruleset name ... markup
      fprintf(doc, _("%sLunar Gambit %s governments help (%s)%s\n\n"),
              tag_info->title_begin, freeciv21_version(), game.control.name,
              tag_info->title_end);
    ...
    case MANUAL_UNITS:
      ...
      // FIXME: this doesn't resemble the wiki manual at all.
      // TRANS: markup ... Freeciv21 version ... ruleset name ... markup
      fprintf(doc, _("%sLunar Gambit %s unit types help (%s)%s\n\n"),
              tag_info->title_begin, freeciv21_version(), game.control.name,
              tag_info->title_end);
    ...
    case MANUAL_TECHS:
      // FIXME: this doesn't resemble the wiki manual at all.
      // TRANS: markup ... Freeciv21 version ... ruleset name ... markup
      fprintf(doc, _("%sLunar Gambit %s tech help (%s)%s\n\n"),
              tag_info->title_begin, freeciv21_version(), game.control.name,
              tag_info->title_end);

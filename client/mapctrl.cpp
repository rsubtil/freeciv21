  ...
  case SC_SELECT_BUTTON:
    ...
    if (king()->rallies.hover_city) {
      ...
      if (ptile && tile_city(ptile)) {
        ...
        output_window_append(0, ftc_client, text);
      } else {
        output_window_append(0, ftc_client, _("#No city selected. Aborted"));
      }
      return;
    }
    ...
    if (king()->menu_bar->quick_airlifting && ptile) {
      if (tile_city(ptile)) {
        ...
        output_window_append(0, ftc_client, "#No city selected for airlift");
      }

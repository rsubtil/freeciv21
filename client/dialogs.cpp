...
void races_dialog::ok_pressed()
{
  ...
  if (selected_sex == -1) {
    output_window_append(0, ftc_client, _("#You must select your sex."));
    return;
  }

  if (selected_style == -1) {
    output_window_append(0, ftc_client, _("#You must select your style."));
    return;
  }

  if (leader_name->currentText().length() == 0) {
    output_window_append(0, ftc_client, _("#You must type a legal name."));
    return;
  }
